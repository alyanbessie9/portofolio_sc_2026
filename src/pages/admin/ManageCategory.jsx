...
export default function ManageCategories() {
  ...
  const handleSave = async (e) => {
    ...
    if (form.id) {
      ...
      else alert("Kategori berhasil diperbarui!");
    } else {
      const { error } = await supabase.from("categories").insert([payload]);
      if (error) alert("Gagal tambah kategori: " + error.message);
      else alert("Kategori baru berhasil ditambahkan!");
    }
    ...
    <div className="text-slate-100 max-w-6xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 pb-6 border-b border-slate-800">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 text-indigo-400 text-xs font-semibold mb-2 border border-indigo-500/20">
            <Sparkles size={13} /> Panel Manajemen Kategori
          </div>
          <h1 className="text-3xl font-black tracking-tight text-slate-100">
            Kelola Kategori
          </h1>
          <p className="text-sm text-slate-400 mt-1">
            Buat, edit, dan kelola data kategori dengan struktur lengkap.
          </p>
        </div>
      </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2 flex items-center gap-1.5">
              <Tag size={14} className="text-indigo-400" /> Kategori Terkait
            </label>
            <input
              type="text"
        ...
        <div>
          ...
          <textarea
            placeholder="Tulis deskripsi pendek kategori..."
        ...
        <div>
          ...
          <div className="rounded-2xl overflow-hidden shadow-inner quill-light-editor">
            <ReactQuill
              ...
              modules={modules}
              placeholder="Tulis konten lengkap kategori di sini..."
            />
          </div>
        </div>
        ...
        <div className="pt-4 flex items-center gap-4">
          ...
          >
            <Plus size={18} />
            {form.id ? "Perbarui Kategori" : "Simpan Kategori Baru"}
          </button>
          {form.id && (
            <button
              type="button"
          ...
          <table className="w-full text-left text-sm text-slate-300">
            <thead className="bg-slate-950 text-slate-400 uppercase text-xs tracking-wider">
              <tr>
                ...
                <th className="p-4.5 font-semibold">Nama Kategori / Judul</th>
                <th className="p-4.5 font-semibold">Sub Kategori</th>
            ...
            <tbody className="divide-y divide-slate-800/60 bg-slate-900/30">
              {categories.length > 0 ? (
                ...
                <tr>
                  ...
                  >
                    Belum ada kategori tersimpan di dalam database.
                  </td>
                </tr>
              )}
            </tbody>
